...
export class SchedulerService {
  ...
  fetchPartitionList(): Observable<Partition[]> {
    const partitionUrl = `${this.envConfig.getYuniKornWebAddress()}/v1/partitions`;
  ...
  fetchSchedulerQueues(partitionName: string): Observable<any> {
    const queuesUrl = `${this.envConfig.getYHSWebAddress()}/v1/partition/${partitionName}/queues`;
  ...
  fetchAppList(partitionName: string, queueName: string): Observable<AppInfo[]> {
    const appsUrl = `${this.envConfig.getYuniKornWebAddress()}/v1/partition/${partitionName}/queue/${queueName}/applications`;
